...
def redirect(url, code=302):
  ...
  # if request.cookies.get('network'):
    url = 'http://%s/%s%s' % (settings.PRIMARY_DOMAIN,
                              request.cookies.get('network'), url)
...
def render_homepage(session_id, title, **kwargs):
  ...
  network = hostname_short[
            :(len(hostname_short) - len(settings.PRIMARY_DOMAIN) - 1)]
...
def search():
  ...
  if item_type in ['people', 'email']:
    ...
    if request.method == 'OPTIONS':
      if query:
        if item_type == 'people':
          ...
          users = [i for i in owner.google_contacts \
                   if api.get_user_id_from_email_address(
              i.email) not in group.member_ids]
      ...
      if group_id:
        out = [i for i in users if
               i.email and i.id not in group.member_ids and i.id != owner.id]
        out.extend([i for i in users if
                    i.email and i.id != owner.id and i.id in group.member_ids])
        users = out
      else:
  ...
  if body.split('<ul id="stream">')[-1].split('<script>')[0].split(
      'right-sidebar')[0].count(query) < 2:
...
@app.route(
  '/<any(discover, starred, hot, incoming, shared_by_me):name>',
  methods=['GET', 'OPTIONS'])
@app.route(
  '/<any(discover, starred, hot, incoming, shared_by_me):name>/page<int:page>',
  methods=['OPTIONS'])
...
def invite():
  ...
  if request.method == 'OPTIONS':
    ...
    if owner.google_contacts is not None:
      email_addrs = [i for i in owner.google_contacts if
                     (i not in member_addrs) and (i not in invited_addrs)]
...
def authentication():
  ...
  if request.path.endswith('sign_in'):
    ...
    if current_network is not None and \
       'auth_normal_whitelist' in current_network and \
       current_network['auth_normal_whitelist'] != "":
      # auth_whitelist = current_network['auth_normal_whitelist'].split(',')
      auth_whitelist = [x.strip() for x in
                        current_network['auth_normal_whitelist'].split(',')]
    ...
    if len(auth_whitelist) > 1 and (not email.split('@')[1] in auth_whitelist):
      flash('Your email is not allowed to sign in this network. '
            'Please contact network administrator for more info.')
      user_url = 'http://%s/%s?error_type=auth_normal' \
                 % (settings.PRIMARY_DOMAIN, network)
      return redirect(user_url)

    session_id = api.sign_in(email, password, user_agent=user_agent,
    ...
    elif session_id is False: # existing user, wrong password
      flash('Wrong password, please try again :)')
      return redirect(user_url + '?error_type=auth_normal')
    elif session_id == -1:
      flash(
        'You used this email address with Facebook login. Please try it again')
      return redirect(user_url + '?error_type=auth_normal')
    elif session_id == -2:
      flash(
        'You used this email address with Google login. Please try it again')
      return redirect(user_url + '?error_type=auth_normal')
    ...
    # (meaning user keyed in something in the whitelist textbox)
    if len(auth_whitelist) > 1 and (not email.split('@')[1] in auth_whitelist):
      flash('Your email is not allowed to sign up this network. '
            'Please contact network administrator for more info.')
      user_url = 'http://%s/%s?error_type=auth_normal' % (
      settings.PRIMARY_DOMAIN, network)
      return redirect(user_url)
    ...
    if request.method == 'OPTIONS':
      ...
      if not email:
        ...
        if r is True:
          message = "We've sent you an email with instructions on how to " \
    ...
    if request.method == 'GET':
      ...
      return redirect(
        '/sign_in?email=%s&msg=Your password has been reset' % email)
...
def google_login():
  ...
  return redirect('https://accounts.google.com/o/oauth2/auth?'
                  ...
                  '&from_login=1&pli=1&login_hint=%s&user_id_1=%s'
                  '&prompt=select_account' % (settings.GOOGLE_REDIRECT_URI,
                                              domain + ";" + network,
...
def google_authorized():
  ...
  # (meaning user keyed in something in the whitelist textbox)
  if network != "" and \
     len(auth_whitelist) > 1 and \
     not user_domain in auth_whitelist:
    flash('Your email is not allowed to login this network. '
          'Please contact network administrator for more info.')
    user_url = 'http://%s/%s?error_type=auth_google' \
               % (settings.PRIMARY_DOMAIN, network)
    return redirect(user_url)
...
def facebook_login():
  ...
  if action == 'import_step_1':
    f.redirect_uri = 'http://%s/oauth/facebook/authorized_import_step_1' \
                     % domain

    auth_url = f.get_auth_url(scope=['email', 'user_groups'])
  else:
    f.redirect_uri = 'http://%s/oauth/facebook/authorized?' \
                     'domain=%s&network=%s' % (domain, domain, network)
...
def facebook_authorized_import_step_2():
  ...
  if 'new_jupo_group_name' in request.args and request.args.get(
      'new_jupo_group_name') != "":
    session_id = session['session_id']
    new_jupo_group_name = request.args.get('new_jupo_group_name')

    # manual create new Jupo group
    target_jupo_group_id = api.new_group(session_id=session_id,
                                         name=new_jupo_group_name,
                                         privacy='open', about=None)
  else:
    target_jupo_group_id = request.args.get('target_jupo_group_id')

  group_that_already_imported_this_fb_group = \
    api.check_exist_imported_facebook_group_id_in_all_group_info(
      source_facebook_group_id=source_facebook_group_id
    )
  print "DEBUG - group_that_already_imported_this_fb_group = " + str(
    group_that_already_imported_this_fb_group)
  ...
  api.importer_queue.enqueue_call(func=api.import_facebook,
                                  args=(session['session_id'], domain, network,
...
def facebook_canvas():
  ...
  decoded_signed_request = parse_signed_request(signed_request,
  ...
  if fb_source == 'jupo_importer':
    ...
    if session and 'session_id' in session:
      ...
      db_name = (invited_network + "." + settings.PRIMARY_DOMAIN).replace('.',
                                                                          '_')

      target_contacts = api.find_target_facebook_contacts_to_invite(
        user_id=user_id, group_id=imported_jupo_group_id, db_name=db_name)
...
def import_data_invite_friends():
  ...
  #    response_noti = requests.post(
  #      'https://graph.facebook.com/%s/notifications'
  #      '?access_token=%s&href=%s&template=%s'
  #      % (contact_fb_id, '1427208454172770|QwcKzFThxruAT11P6TQTlIeDz_s',
  #         'http://jupo.localhost.com/jupo.com', 'Hello world from Jupo'))
  #
  #  resp = Response(render_template('import_invite.html',
  #                                  email=email,
  #                                  settings=settings,
...
def facebook_authorized():
  ...
  if settings.FACEBOOK_APP_ID and settings.FACEBOOK_APP_SECRET:
    f = FacebookAPI(
      client_id=settings.FACEBOOK_APP_ID,
      client_secret=settings.FACEBOOK_APP_SECRET,
      redirect_uri='http://%s/oauth/facebook/authorized'
                   '?domain=%s&network=%s' % (domain, domain, network)
    )
  ...
  session_id = api.sign_in_with_facebook(
    email=email,
    name=me.get('name'),
  ...
  resp = redirect(url)
  # set this so that home() knows which network user just signed up,
  # same as in /oauth/google/authorized
  resp.set_cookie('network', network)
...
def user(user_id=None, page=1, view=None):
  ...
  #         return redirect('/?message=New password updated successfully.')
    ...
    if birthday_day.isdigit() and \
       birthday_month.isdigit() and \
        birthday_year.isdigit():
      info['birthday'] = '%s/%s/%s' % (birthday_day,
                                       birthday_month,
                                       birthday_year)
    ...
    if request.method == 'POST':
      ...
      if domain:
        if '.' not in domain:
          ...
          msg = '''
            Please make sure <strong>%s</strong> is mapped to
            <strong>play.jupo.com</strong>.<br>
            ...
            until the changes take effect around the world.
          ''' % domain
...
def group(group_id=None, view='group', page=1):
  ...
  if not group_id:
    ...
    _default_groups = [
      ...
      {'name': 'Design',
       'description': 'Design is now how it looks. '
                      'Design is what the boss likes'},
      {'name': 'Customer Services',
       'description': 'Getting yelled at for things '
  ...
  if action == 'new_message':
    ...
    if user_id:
      ...
      if not timestamp:
        ...
        last_viewed_friendly_format = api.friendly_format(last_viewed,
                                                          short=True)
        if last_viewed_friendly_format.startswith('Today'):
          last_viewed_friendly_format = \
            last_viewed_friendly_format.split(' at ')[-1]

        if messages and (messages[-1].sender.id == owner_id) and (
          messages[-1].timestamp < last_viewed):
...
@app.route("/feed/<int:feed_id>/<message_id>@<domain>",
           methods=["GET", "OPTIONS"])
...
def feed_actions(feed_id=None, action=None,
  ...
  if not user_id:
    if not request.path.startswith('/post/'):
      ...
      url_redirect = 'http://%s/%s%s' % (
      settings.PRIMARY_DOMAIN, network, request.path)
  ...
  if request.path.endswith('new'):
    ...
    if request.method == 'OPTIONS':
      ...
      if feed.is_note():
        ...
        msg = filters.clean(feed.message)
        if not msg or \
            (msg and str(msg).strip()[0] == '{' and str(msg).strip()[
              -1] == '}'):
          title = ''
        else:
          title = msg if len(msg) <= 50 else msg[:50] + '...'
      ...
      if request.path.startswith('/post/') or not owner.id:
        return render_template('post.html',
                               network=request.headers.get('Host')[
                                       :-(len(settings.PRIMARY_DOMAIN) + 1)],
                               owner=owner,
                               mode='view',
                               settings=settings,
                               title=title,
...
def event(event_id=None, group_id=None):
  ...
  if request.path == '/event/new':
    ...
    if viewers:
      viewers = viewers.split(',')
    event_id = api.new_event(session_id, name, when, where, None, details,
                             viewers)
...
def attachment(attachment_id=None, action=None):
  ...
  if request.path.endswith('new'):
    ...
    if not name and not content_type.startswith('image/'):
      name = filename
    if name:
      resp.headers['Content-Disposition'] = str(
...
def profile_pictures(attachment_id, size='60'):
  try:
    ...
    if info.md5 and api.is_s3_file(info.md5 + '_%s.jpg' % size):
      url = 'https://%s.s3.amazonaws.com/%s_%s.jpg' % (
      settings.S3_BUCKET_NAME, info.md5, size)
...
class NetworkNameDispatcher(object):
  ...
  def __call__(self, environ, start_response):
    ...
    if '.' in items[0] and api.is_domain_name(items[0]):  # is domain name
      ...
      network = request.cookies.get('network')
      if not network or not api.is_domain_name(network) or (
        network == 'favicon.ico'):
        return self.app(environ, start_response)
