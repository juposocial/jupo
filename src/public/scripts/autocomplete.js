...
(function($){
    ...
    function createList(data){
        $('ul.auto-list').remove(); // remove old if exists
    ...
    function createClone(id){
        var data = _data[id];
        $('div.textarea').remove(); // remove old if exists
    ...
    function showList(data,list,text){
        ...
        for( var i=0; i< list.length; i++ ){
            ...
            html += "<li data-id='" + list[i].id + "' data-value='"+list[i].name+"' "+width+">"+list[i].name.replace(regEx,"<b>$1</b>")+"</li>";
        }
        $(data.list).html(html);
    }   
    ...
    function onUserSelected(li,data){
        ...
        item = '{"id": "' + selectedID + '", "name": "' + selectedText + '"},'
        users = $(data.selected_items).val() + item;
        $(data.selected_items).val(users);
        
        console.log($(data.selected_items).val());
            ...
            if( text != "" ){
                data.on.query(text,function(list){
                    ...
                    if( list.length ){
                        ...
                        setSelected(list.length,data);  // auto select first item
                    }
                    else{
                        hideList(data);
                    }
