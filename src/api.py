...
from lib.pbkdf2 import pbkdf2_bin   # https://github.com/mitsuhiko/python-pbkdf2
...
def send_mail(to_addresses, subject=None, body=None, mail_type=None,
  ...
  # support subdir instead of subdomain
  # (jupo.com/your-net-work instead of your-network.jupo.com)
  ...
  if mail_type == 'thanks':
    ...
    template = app.CURRENT_APP.jinja_env.get_template(
    ...
    body = template.render(domain=domain, email=to_addresses, user=user,
                           post=post)
  ...
  if to_addresses and subject and body:
    ...
    if post:
      ...
      mail_id = base64.b64encode(smaz.compress(mail_id))\
                      .replace('/', '-').rstrip('=')
...
def image_data_uri(filename):
  ...
  data = "data:%s;base64,%s" % (mimetype,
                                base64.encodestring(content).replace('\n', ''))
  return data
...
def datetime_string(timestamp):
  ...
  s = s.replace(' 0', ' ').replace('AM', 'am').replace('PM', 'pm')
  return s
...
def get_url_description(url, bypass_cache=False, db_name=None):
  ...
  # only refresh after 90 days
  if url_info and \
     not bypass_cache and \
     abs(utctime() - url_info.get('timestamp')) < 86400 * 90:
    return url_info
...
def get_user_id(session_id=None, facebook_id=None, email=None, db_name=None):
  ...
  key = '%s:%s:uid' % (db_name, session_id
                                if session_id
                                else facebook_id
                                if facebook_id
                                else email)
...
def get_session_id_by_email(email, db_name=None):
  ...
  elif 'password' not in user:  # user chưa tồn tại, tạo tạm 1 session_id
    session_id = hashlib.md5('%s|%s' % (user['_id'], utctime())).hexdigest()
...
def update_imported_facebook_group_id_in_group_info(target_jupo_group_id,
                                                    source_facebook_group_id,
                                                    db_name):
  db = DATABASE[db_name]
  ...
  if target_jupo_group_id != 'public':
    db.owner.update({'_id': long(target_jupo_group_id)},
                    {'$addToSet': {
                    'imported_facebook_group_ids': source_facebook_group_id}})


def check_exist_imported_facebook_group_id_in_all_group_info(
    source_facebook_group_id, db_name=None):
  if not db_name:
    db_name = get_database_name()
  ...
  group = db.owner.find_one(
    {'email': None, 'imported_facebook_group_ids': source_facebook_group_id})

  return group


def import_facebook(session_id, domain, network, facebook_token,
                    source_facebook_group_id, target_jupo_group_id,
                    import_comment_likes):
  facebook_import = GraphAPI(facebook_token)

  db_name = (network + '.' + domain).replace('.', '_')

  # record this to avoid duplicate importing
  update_imported_facebook_group_id_in_group_info(target_jupo_group_id,
                                                  source_facebook_group_id,
                                                  db_name)
        ...
        for post in current_group_feeds['data']:
          if counter < 5000:
            ...
            if not poster_session_id:
              ...
              poster_session_id = add_dummy_user(name=poster['name'],
                                                 fb_id=user_fb_id,
                                                 avatar=poster_avatar_url,
                                                 db_name=db_name)

            if target_jupo_group_id != 'public':
              join_group(poster_session_id, target_jupo_group_id,
                         db_name=db_name, is_import=True)

            new_feed_id = check_feed_exist_by_fb_id(fb_id=post["id"],
                                                    group_id=target_jupo_group_id,
                                                    db_name=db_name)
            if not new_feed_id:
              ...
              if post['type'] == 'photo':
                ...
                attached_photo_id = new_attachment(poster_session_id,
                                                   image_name, urllib.urlopen(
                    image_url).read(), db_name)
              ...
              new_feed_id = new_feed(poster_session_id,
                                     post[
                                       'message'] if 'message' in post else None,
                                     #['public'],
                                     [target_jupo_group_id],
                                     attachment,
                                     None,
                                     float(dateparser.parse(
                                       post['updated_time']).strftime('%s.%f')),
                                     float(dateparser.parse(
                                       post['created_time']).strftime('%s.%f')),
                                     db_name,
                                     "True",
                                     post["id"])
              ...
              if 'likes' in post:
                for l in post['likes']['data']:
                  ...
                  like_user_session_id = add_dummy_user(name=l['name'],
                                                        fb_id=like_user_fb_id,
                                                        db_name=db_name)

                  like(like_user_session_id, new_feed_id, new_feed_id, db_name,
                       "True")
            ...
            if 'comments' in post:
              for comment in post['comments']['data']:
                ...
                comment_poster_session_id = check_user_exist_by_fb_id(
                  comment_user_fb_id, db_name)
                ...
                if not comment_poster_session_id:
                  ...
                  comment_poster_session_id = add_dummy_user(
                    name=comment['from']['name'], fb_id=comment_user_fb_id,
                    avatar=comment_poster_avatar_url, db_name=db_name)

                if target_jupo_group_id != 'public':
                  join_group(comment_poster_session_id, target_jupo_group_id,
                             db_name=db_name, is_import=True)

                new_comment_obj = check_comment_exist_by_fb_id(
                  feed_id=new_feed_id, fb_id=comment['id'], db_name=db_name)

                if not new_comment_obj:
                  new_comment_obj = new_comment(
                    session_id=comment_poster_session_id,
                    message=comment['message'], ref_id=new_feed_id,
                    attachments=None, reply_to=None, from_addr=None,
                    db_name=db_name, updated_time=float(
                      dateparser.parse(comment['created_time']).strftime(
                        '%s.%f')), created_time=float(
                      dateparser.parse(comment['created_time']).strftime(
                        '%s.%f')), is_import="True", fb_id=comment["id"])

                # import like comment
                if import_comment_likes == 'on':
                  current_comment = facebook_import.get(
                    post['id'] + '_' + comment['id'],
                    params={'fields': 'likes'})
                  print "DEBUG - in api.import_facebook - current_comment = " + str(
                    current_comment)
                  ...
                  if 'likes' in current_comment:
                    for like_comment in current_comment['likes']['data']:
                      ...
                      like_comment_user_session_id = add_dummy_user(
                        name=like_comment['name'],
                        fb_id=like_comment_user_fb_id, db_name=db_name)

                      like(like_comment_user_session_id, new_comment_obj.id,
                           new_feed_id, db_name, "True")
  ...
  print "DEBUG - in api.import_facebook - about to enqueue with network = " + str(
    network)
  notification_queue.enqueue(new_notification,
                             session_id, current_user_id,
                             'import_completed',
                             None, None, db_name=db_name, network=network,
                             imported_jupo_group_id=target_jupo_group_id)

  return "True"


def find_target_facebook_contacts_to_invite(group_id=None, user_id=None,
                                            db_name=None):
  if not db_name:
    db_name = get_database_name()
  db = DATABASE[db_name]
...
def move_to_s3(fid, db_name=None):
  ...
  if (isinstance(fid, str) or isinstance(fid, unicode)) and '|' in fid:
    ...
    f = datastore.get(fid)
    filename = f.filename
    is_ok = s3write(filename, f.read(), overwrite=True,
  ...
  if '|' in status:
    ...
    if chat_id.startswith('user-'):
      ...
      push_queue.enqueue(publish, int(uid), 'typing-status',
                         {'chat_id': 'user-%s' % user_id, 'text': text},
                         db_name=db_name)
    else:
...
def check_status(user_id, db_name=None):
  ...
  if status == 'online' and (utctime() - last_online(
      user_id) > 5 * 60):  # quá 5 phút mà vẫn thấy online thì chắc là lỗi rồi
    status = 'offline'
  return status
...
def get_user_info(user_id=None, facebook_id=None, email=None, db_name=None):
  ...
  key = '%s:info' % (
  user_id if user_id else facebook_id if facebook_id else email)
  info = cache.get(key)
  if not info:
    if facebook_id:
...
def new_notification(session_id, receiver, type,
                     ref_id=None, ref_collection=None, comment_id=None,
                     db_name=None, **kwargs):
  if not db_name:
    db_name = get_database_name()
  db = DATABASE[db_name]

  print "DEBUG - in api.new_notification - network = " + str(
    kwargs.get('network'))
  ...
  info = {'_id': new_id(),
          ...
          'is_unread': True,
          'timestamp': utctime(),
          'network': kwargs.get('network') if kwargs.has_key(
            'network') else None,
          'imported_jupo_group_id': kwargs.get(
            'imported_jupo_group_id') if kwargs.has_key(
            'imported_jupo_group_id') else None}
...
def mark_as_read(session_id, post_id, db_name=None):
  ...
  if (post.last_action.owner.id != user_id and
    ...
    # update notification
    mark_notification_as_read(session_id=session_id, ref_id=post_id,
                              db_name=db_name)
...
def get_member_email_addresses(db_name=None):
  ...
  member_email_addresses = [i['email'] \
                            for i in db.owner.find(
      {'email': {'$ne': None}, 'name': {'$ne': None}},
...
def new_feed(session_id, message, viewers,
             attachments=[], facebook_access_token=None, updated_time=None,
             created_time=None, db_name=None, is_import=None, fb_id=None):
  if not db_name:
    db_name = get_database_name()
  db = DATABASE[db_name]
  ...
  if not is_import:
    for id in viewers:
      if not is_group(id, db_name) and id != user_id:
        ...
        if u.email and 'share_posts' not in u.disabled_notifications and '@' in u.email and (
            u.status == 'offline' or (
...
def set_viewers(session_id, feed_id, viewers, db_name=None):
  ...
  if not feed:
    return False
  new_users = [i for i in viewers if
               not is_group(i, db_name) and i not in feed['viewers']]
  for i in new_users:
    if i != user_id:
      notification_queue.enqueue(new_notification,
...
def reshare(session_id, feed_id, viewers):
  ...
  if not feed:
    return False
  new_users = [i for i in viewers if
               not is_group(i) and i not in feed['viewers']]
  for i in new_users:
    if i != user_id:
      notification_queue.enqueue(new_notification,
                                 session_id, i, 'message',
...
def get_hot_posts(page=1):
  ...
  feeds = feeds[((page - 1) * settings.ITEMS_PER_PAGE):(
  page * settings.ITEMS_PER_PAGE)]
  return [Feed(i, db_name=db_name) for i in feeds if i]
...
def new_comment(session_id, message, ref_id,
                ...
                updated_time=None, created_time=None, is_import=None,
                fb_id=None):
  if not db_name:
    db_name = get_database_name()
  db = DATABASE[db_name]
  ...
  if not is_import:
    if user_id != owner_id:
      notification_queue.enqueue(new_notification,
                                 ...
                                 ref_id=ref_id, comment_id=comment['_id'],
                                 db_name=db_name)
  ...
  if not is_import:
    if owner.email and '@' in owner.email and 'comments' not in owner.disabled_notifications and user_id not in mentions:
      if owner.status == 'offline' or (
          owner.status == 'away' and utctime() - owner.last_online > 180):
  ...
  if not is_import:
    mark_notification_as_read(session_id=session_id, ref_id=ref_id,
                              db_name=db_name)
...
def compare_with_previous_version(session_id, doc_id, revision):
  ...
  try:
    prev_title = info.get('version')[revision + 1].get('title',
                                                       'Untitled Noted')
  except:
    prev_title = None
...
def update_note(session_id, doc_id, title, content, attachments=None,
                viewers=None):
  db_name = get_database_name()
  db = DATABASE[db_name]
...
def check_user_exist_by_fb_id(fb_id, db_name=None):
  db = DATABASE[db_name]
  existing_user = db.owner.find_one(
    {'$or': [{'fb_id': fb_id}, {'facebook_id': fb_id}]})
  if existing_user:
...
def check_comment_exist_by_fb_id(feed_id, fb_id, db_name=None):
  db = DATABASE[db_name]
  existing_comment = db.stream.find_one(
    {'_id': feed_id, 'comments.fb_id': fb_id})
  if existing_comment:
...
def get_attachments(session_id, group_id=None, limit=10):
  ...
  for post in posts:
    ...
    elif group_id and post.has_key('history') and post['history'][0].has_key(
        'attachment_id'):
...
def get_group_info(session_id, group_id, db_name=None):
  ...
  if (info and info.get('privacy') in ['open', 'closed']) \
    ...
    if not info.has_key(
        'recently_viewed'): # preinitialize recently_viewed arrays with nulls
      db.owner.update({'_id': group_id},
                      {'$set':
...
def search(session_id, query, type=None, ref_user_id=None, page=1):
  ...
  query_dsl['sort'] = [{'ts': {'order': 'desc'}},
                       ...
                       "_score"]
  query_dsl["facets"] = {
  "counters": {"terms": {"field": "type", "all_terms": "true"}},
    ...
    for i in info.get('read_receipts'):
      if i.get('timestamp') > last_updated:
        read_receipts.append(
          {'user': get_user_info(i['user_id'], db_name=db_name),
...
def like(session_id, item_id, post_id=None, db_name=None, is_import=None):
  ...
  if not is_import:
    if post_id == item_id:  # like post
      ...
      likes = [get_user_info(user_id=i, db_name=db_name) for i in
               get_liked_user_ids(post_id, db_name)]
      record['likes'] = likes
      for uid in viewers:
      ...
      likes = [get_user_info(user_id=i, db_name=db_name) for i in
               get_liked_user_ids(item_id, db_name)]
      comment['likes'] = likes
      for uid in viewers:
        push_queue.enqueue(publish, uid, 'like-comment', comment,
                           db_name=db_name)
...
def new_network(db_name, organization_name, description=None):
  ...
  info = {'domain': db_name.replace('_', '.'),
          ...
          'auth_google': 'on',
          # activate Google authentication for new network by default
          'timestamp': utctime()}
...
def get_networks(user_id, user_email=None):
  ...
  for db_name in db_names:
    ...
    if info:
      ...
      info['unread_notifications'] = get_unread_notifications_count(user_id,
                                                                    db_name=db_name)
      # info['domain'] = db_name.replace('_', '.')
      if 'domain' in info:
        hostname = info['domain']
        network_url = hostname[
                      :(len(hostname) - len(settings.PRIMARY_DOMAIN) - 1)]
...
def get_messages(session_id, archived=False, db_name=None):
  ...
  messages = [Message(i, utcoffset=utcoffset, db_name=db_name) for i in
              messages]
...
@line_profile
def get_unread_messages_count(session_id, user_id=None, topic_id=None,
                              db_name=None):
  if not db_name:
    db_name = get_database_name()
  db = DATABASE[db_name]
  ...
  if user_id:
    ...
    for record in records:
      ...
      count += get_unread_messages_count(session_id, topic_id=topic_id,
                                         db_name=db_name)
...
@line_profile
def get_chat_history(session_id, user_id=None, topic_id=None, timestamp=None,
                     db_name=None):
  if not db_name:
    db_name = get_database_name()
  db = DATABASE[db_name]
  ...
  for record in records:
    ...
    if last_msg and \
        ...
        msg = "<a href='/attachment/%s' target='_blank' title='%s (%s)' %s>%s (%s)</a>" \
              % (attachment.id, attachment.name, attachment.size,
                 '' if attachment.mimetype.startswith(
                   'image/') else "download='%s'" % attachment.name,
      ...
      if is_snowflake_id(msg):
        ...
        last_msg[
          'text'] = "<a href='/attachment/%s' target='_blank' title='%s (%s)' %s>%s (%s)</a>" \
                    % (attachment.id, attachment.name, attachment.size,
                       '' if attachment.mimetype.startswith(
                         'image/') else "download='%s'" % attachment.name,
                       attachment.name, attachment.size)
...
def push_notification_to_mobile(sender_id=None, receiver_id=None, info=None,
                                db_name=None):
  ...
  if info:
    ...
    if list_device_token_android:
      status_push = push_nofitication_to_google(
        list_device_token=list_device_token_android,
        message=msg)

    if list_device_token_apple:
      status_push = push_notification_to_apple(
        list_device_token=list_device_token_apple,
